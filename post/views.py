...
def category_search(request):
    template= 'category_search.html'
    queryset = Post.objects.all()
    query = request.GET.get('qry')
    catset = {}
    res = Post.objects.raw('select pp.id from post_post pp inner join post_post_categories ppc on ppc.post_id = pp.id inner join post_category pc on pc.id=ppc.category_id where pc.title= %s', [query])
    x=[]
    for catset in res:
        x.append(catset.id)
    queryset = queryset.filter(
